...
//die("broken for now lol");
  ...
  if(base64_decode($image64)) {
    ...
    $ID = $id;
    $imageFile = fopen($_SERVER["DOCUMENT_ROOT"]."/avatar/render/avatars/$ID.png", "w") or die("Unable to write avatar!");
    $data = $img;
    if ($imageFile) {
      ...
      if ($writeImage) {
        ...
        ?> <script> window.location = "/avatar/render/avatars/<?php echo $ID; ?>.png?c=<?php echo rand() ?>"; </script>
        <?php
        die();
  ...
  //hat 4
    ...
    $hat7ID = $avatar->{"head"};
    $shopSQL7 = "SELECT * FROM `shop_items` WHERE  `id` = '".$headID."' ";
    $result7 = $conn->query($shopSQL7);
    if ($result7->num_rows > 0) {
      $shopRow7 = (object) $result5->fetch_assoc();
    }else{
      $shopRow7 = 0;
    }
  ...
  if($toolID > 0) {
   ...
   $zoom = min($zoomF1,$zoomF2,$zoomF3,$zoomF4,$zoomF5,$zoomF6) - 0.4;
   if($shopRow6->{'zoom'} > 0) {
...
<html>
  ...
  <body>
    <img src="/assets/GeneratingAvatar.gif" style="height: 285px;padding-top: 15px;margin-left: -25px;">
    <canvas id="avatarCanvas" width="235px" height="280px" style="display:none;"></canvas>
    <script src="/assets/render/three.min.js"></script>
    <script src="/assets/render/stats.min.js"></script>
    <script src="/assets/render/OBJLoader.js"></script>
  <form action="" method="POST">
    ...
    scene.add( directionalLight );
        ...
        var headtex = new THREE.Texture();

        // Load textures
        var loader = new THREE.ImageLoader( manager );
        loader.load( '/shop_storage/assests/faces/<?php if ($avatar->{"face"} > 0) { echo shopItemHash($avatar->{"face"}); } else { echo "facez"; }; ?>.png' , function ( faceimg ) {
        ...
        loader.load( '/shop_storage/assests/heads/<?php echo shopItemHash($avatar->{"head"}); ?>.png' , function ( headimg ) {
          headtex.image = hat1img;
          headtex.needsUpdate = true;
        } );
        // model
        var loader = new THREE.OBJLoader( manager );
        loader.load( '<?php if($avatar->{"head"} > 0) { echo "/shop_storage/assests/heads/"; ?><?php echo shopItemHash($avatar->{"head"}); ?><?php echo ".obj" ;} else { echo "../Head.obj"; } ?>', function ( head ) {
