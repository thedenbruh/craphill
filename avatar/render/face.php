...
include('../../SiT_3/PHP/helper.php');
  ...
  'torso_color' => 'ffffff','left_leg_color' => 'd1d1d1','right_leg_color' => 'd1d1d1','left_arm_color' => 'f3b700','right_arm_color' => 'f3b700','head_color' => 'A9A9A9','head' => 0);
  ...
  if(base64_decode($image64)) {
    ...
    $ID = $_SESSION['id'];
    $imageFile = fopen("../../shop_storage/thumbnails/$itemID.png", "w") or die("Unable to open file!");
    $data = $img;
    if ($imageFile) {
      ...
      if ($writeImage) {
        ...
        ?> <script> window.location = "/shop_storage/thumbnails/<?php echo $itemID; ?>.png?c=<?php echo rand() ?>";</script>
        <?php
        die();
...
<html>
  ...
  <form action="" method="POST">
    ...
    <script>
      ...
      var techLight = new THREE.AmbientLight(0xfcfcfc, 1);
      scene.add(techLight);
      function init() {
        container = document.createElement( 'div' );
        document.body.appendChild( container );
        camera = new THREE.PerspectiveCamera(70, 225/280, 0.1, 1000 );
    ...
    var directionalLight = new THREE.AmbientLight( 0xbbbbbb, 0.25 );
   // directionalLight.position.set( 0, 2, 1 );
    scene.add( directionalLight );
        ...
        loader.load( '<?php if($avatar->{"head"} > 0) { echo "/shop_storage/assests/heads/"; ?><?php echo shopItemHash($avatar->{"head"}); ?><?php echo ".obj" ;} else { echo "face.obj"; } ?>', function ( head ) {
        ...
        */
        ...
        loader.load( 'TShirt.obj', function ( tShirt ) {
          tShirt.traverse( function ( child ) {
            if ( child instanceof THREE.Mesh ) {
              //child.material.color.setHex( 0xD1D1D1 );
