<?php
  ...
  if(isset($_POST['newName'])) {
    ...
    $desc = mysqli_real_escape_string($conn,$_POST['name']);
    $updateSQL = "UPDATE `clans` SET `name`='$desc' WHERE `id`='$clanID'";
    $update = $conn->query($updateSQL);
  }
  ...
  if(isset($_POST['payout'])) {
    $currentperson = "SELECT * FROM `beta_users` WHERE `id`='$userID'";
    $testa = $conn->query($currentperson);
    $cocurrentrow = $testa->fetch_assoc();
  
    if($cocurrentrow['bucks'] >= 10){
      $Payamount = mysqli_real_escape_string($conn,$_POST['amount']);
      $Usernamee = mysqli_real_escape_string($conn,$_POST['user']);
      $payval= (is_numeric($_POST['Amount']));
      $usera = mysqli_real_escape_string($conn,$_POST['user']);
      $sqlusera = "SELECT * FROM `beta_users` WHERE `id`='$usera'";
      $resulte = $conn->query($sqlusera);
      $otheruserRow = $resulte->fetch_assoc();
   
          if($Payamount <= $clanRow['funds']) {
          $newBucksx = $clanRow['funds']-$Payamount;
          $newplayerbucks = $otheruserRow['bucks']+$Payamount;
          $newBucksSQLx = "UPDATE `clans` SET `funds`='$newBucksx' WHERE `id`='$clanID'";
          $newBucksSQLxs = "UPDATE `beta_users` SET `bucks`='$newplayerbucks' WHERE `id`='$usera'";

          $buys = $conn->query($newBucksSQLx);
          $buyss = $conn->query($newBucksSQLxs);
    ...
    if(isset($_FILES['image'])) {
      ...
      if($isImage !== false) {
        if($imgSize < 2097152) {
          ...
          if($approved) {
            move_uploaded_file($imgTmp,"../clans/icons/".$clanID.".png");
          }
        } else {
...
<!DOCTYPE html>
  <head>
    <title>Edit Group - <?php echo $sitename; ?></title>
  </head>
  <body>
    <div id="body">
      <div id="box">
        ...
        <form action="" method="POST" style="margin:10px;" enctype="multipart/form-data">
          <h4>Image:</h4>
          <img style="width:200px;height:200px;" src="/clans/icons/<?php echo $clanID; ?>.png">
       ...
       <?php /*echo'
        <form action="" method="POST" style="margin:10px;">
          <h4>Name:</h4>
           <input type="text" name="name" style="font-size:14px;padding:4px;margin-bottom:10px;" placeholder="'.$clanRow['name'].'"><br>
  ...
  ?>
      ...
      <div id="box" style="margin-top:10px;">
        ...
        <div id="subsect" style="padding:10px;">
          ...
          <?php /*echo' <div id="paying" style="padding:10px;">
          <h5>Clan Payouts</h5>
          <form action="" method="POST">
            <input name="amount" placeholder="Amount">
            <input name="user" placeholder="User"><br>
            <span style="color:green;font-size:12px;">Payout Members!<i class="fa fa-money"></i></span><br>
            <input type="submit" name="payout">
          </form>
        </div>';*/
          ?>
