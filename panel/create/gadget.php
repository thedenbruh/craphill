        ...
        if($imgSize < 2097152) {
          if(isset($_POST['name'])) {
            if(isset($_POST['bits']) or isset($_POST['bucks'])) {
              ...
              $newSQL = "INSERT INTO `shop_items` (`id`,`owner_id`,`name`,`description`,`bucks`,`bits`,`type`,`date`,`last_updated`,`offsale`,`collectible`,`collectable-edition`,`collectible_q`,`zoom`,`approved`) VALUES (NULL,'$ownerID','$name','$desc','$bucks','$bits','tool','$today','$today','no','$collectible','$collectible','$stock',NULL,'yes')";
              ...
              move_uploaded_file($imgTmp,$_SERVER["DOCUMENT_ROOT"]."/shop_storage/assests/tools/".shopItemHash($itemID).".png");
              move_uploaded_file($objTmp,$_SERVER["DOCUMENT_ROOT"]."/shop_storage/assests/tools/".shopItemHash($itemID).".obj");
...
<!DOCTYPE html>
  <head>
    <title>Create Gadget - <?php echo $sitename; ?></title>
  </head>
  <body>
...
<li class="breadcrumb-item active">Create New Gadget</li>
</ul>
<div class="card">
<div class="card-body">
...
<input class="form-control mb-2" type="number" name="bits" placeholder="Cubes" min="0" max="1000000">
  <input class="form-control mb-2" type="number" name="bucks" placeholder="Cash" min="0" max="1000000">
</div>
</div>
