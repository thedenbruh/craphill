      ...
      if($imgSize < 2097152) {
        if(isset($_POST['name'])) {
          if(isset($_POST['bits']) or isset($_POST['bucks'])) {
            ...
            $newSQL = "INSERT INTO `shop_items` (`id`,`owner_id`,`name`,`description`,`bucks`,`bits`,`type`,`date`,`last_updated`,`offsale`,`collectible`,`collectable-edition`,`collectible_q`,`zoom`) VALUES (NULL,'$ownerID','$name','$desc','$bucks','$bits','shirt','$today','$today','no','no','no','0',NULL)";
            ...
            move_uploaded_file($imgTmp,"../../shop_storage/assests/shirts/".shopItemHash($itemID).".png");
            echo'<script>location.replace("../../../shop/item?id='.$itemID.'&srender");</script>';
          } else {
...
<!DOCTYPE html>
  <head>
    <title>Create Shirt - <?php echo $sitename; ?></title>
  </head>
  <body>
...
<div class="content">
        ...
        <img src="template.png?c=1" style="float:right;width:50%;height:50%px;margin:10px;">
      </div>
    </div>
      </div>
    </div>
